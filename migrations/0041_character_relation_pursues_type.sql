-- Add a directional pursuit edge for source-scoped celestial and chase narratives.
-- Rebuild because SQLite CHECK constraints cannot be altered in place.

CREATE TABLE character_relations__pursues_type (
  id TEXT PRIMARY KEY,
  ...
  relation_type TEXT NOT NULL
    CHECK (relation_type IN (
      ...
      'narrative', 'punishes', 'orders-creation', 'defeats',
      'exchanges-with', 'opposes', 'captures', 'aids', 'rides',
      'companion', 'encounters', 'resists', 'departs-from', 'pursues'
    )),
...
INSERT INTO character_relations__pursues_type (
...
ALTER TABLE character_relations__pursues_type RENAME TO character_relations;

CREATE INDEX idx_character_relations_from ON character_relations(from_character_id, status);
CREATE INDEX idx_character_relations_to_character ON character_relations(to_character_id, status);
CREATE INDEX idx_character_relations_to_concept ON character_relations(to_concept_id, status);
CREATE UNIQUE INDEX idx_character_relations_canonical_assertion
  ON character_relations(
    from_character_id, IFNULL(to_character_id, ''), IFNULL(to_concept_id, ''),
    IFNULL(from_interpretation_id, ''), IFNULL(to_interpretation_id, ''),
    relation_type, tradition_scope
  ) WHERE status = 'active';
CREATE INDEX idx_character_relations_default ON character_relations(from_character_id, is_default, status);
...
FOR EACH ROW WHEN (
  (NEW.from_interpretation_id IS NOT NULL AND NOT EXISTS (
    SELECT 1 FROM character_interpretations WHERE id = NEW.from_interpretation_id AND character_id = NEW.from_character_id
  ))
  OR (NEW.to_interpretation_id IS NOT NULL AND (
    NEW.to_character_id IS NULL OR NOT EXISTS (
      SELECT 1 FROM character_interpretations WHERE id = NEW.to_interpretation_id AND character_id = NEW.to_character_id
    )
  ))
)
BEGIN
...
FOR EACH ROW WHEN NEW.to_character_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM characters AS source
    JOIN characters AS target ON target.id = NEW.to_character_id
    WHERE source.id = NEW.from_character_id AND source.mythology_id = target.mythology_id
  )
BEGIN
