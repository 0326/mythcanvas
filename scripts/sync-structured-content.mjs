...
const bundles = packagePaths.map((slug) => {
  ...
  const objects = loadTsModule(`src/content/${slug}/objects.ts`, true);
  ...
  return { slug, catalog, stories: stories[`${slug}Stories`], mythology, identities, sources, objects };
});
...
function buildSqlForBundle({ characters, worlds, scenes, relations, contentRelations = [], objects = [], taxonomy, stories, mythology, concepts = [], claims = [], names = [], interpretations = [], sources = [] }) {
  ...
  for (const item of objects) statements.push(`INSERT INTO mythic_objects (id, mythology_id, slug, name, name_en, native_name, aliases_json, object_type, summary, tradition_tags_json, source_refs_json, canonical_design_json, hero_src, hero_alt, hero_width, hero_height, status) VALUES (${q(item.id)}, ${q(item.mythologyId)}, ${q(item.slug)}, ${q(item.name)}, ${q(item.nameEn)}, ${nullable(item.nativeName)}, ${json(item.aliases ?? [])}, ${q(item.objectType)}, ${q(item.summary)}, ${json(item.traditionTags ?? [])}, ${json(item.sourceRefs)}, ${json(item.canonicalDesign)}, ${nullable(item.heroImage?.src)}, ${nullable(item.heroImage?.alt)}, ${item.heroImage?.width ?? 'NULL'}, ${item.heroImage?.height ?? 'NULL'}, 'active') ON CONFLICT(id) DO UPDATE SET mythology_id=excluded.mythology_id, slug=excluded.slug, name=excluded.name, name_en=excluded.name_en, native_name=excluded.native_name, aliases_json=excluded.aliases_json, object_type=excluded.object_type, summary=excluded.summary, tradition_tags_json=excluded.tradition_tags_json, source_refs_json=excluded.source_refs_json, canonical_design_json=excluded.canonical_design_json, hero_src=excluded.hero_src, hero_alt=excluded.hero_alt, hero_width=excluded.hero_width, hero_height=excluded.hero_height, status='active';`);
  ...
  for (const item of taxonomy ?? []) {
    const legacyKind = ['lineage', 'domain', 'story-cycle', 'editorial-collection'].includes(item.kind) ? item.kind : 'lineage';
    statements.push(`INSERT INTO taxonomy_terms (id, mythology_id, slug, kind, semantic_kind, name, name_en, summary, display_order, status) VALUES (${q(item.id)}, ${q(item.mythologyId)}, ${q(item.slug)}, ${q(legacyKind)}, ${q(item.kind)}, ${q(item.name)}, ${nullable(item.nameEn)}, ${q(item.summary)}, ${item.displayOrder}, 'active') ON CONFLICT(id) DO UPDATE SET slug=excluded.slug, kind=excluded.kind, semantic_kind=excluded.semantic_kind, name=excluded.name, name_en=excluded.name_en, summary=excluded.summary, display_order=excluded.display_order, status='active';`);
  }
  ...
  for (const object of objects) for (const item of object.sourceRefs ?? []) collectSource(item);
  ...
  for (const relation of contentRelations) for (const item of relation.sourceRefs ?? []) collectSource(item);
  ...
  for (const relation of contentRelations) statements.push(`INSERT INTO content_relations (id, mythology_id, from_type, from_id, to_type, to_id, relation_type, tradition_scope, confidence, source_refs_json, status) VALUES (${q(relation.id)}, ${q(relation.mythologyId)}, ${q(relation.from.type)}, ${q(relation.from.id)}, ${q(relation.to.type)}, ${q(relation.to.id)}, ${q(relation.relationType)}, ${q(relation.traditionScope ?? '')}, ${q(relation.confidence)}, ${json(relation.sourceRefs)}, 'active') ON CONFLICT(id) DO UPDATE SET mythology_id=excluded.mythology_id, from_type=excluded.from_type, from_id=excluded.from_id, to_type=excluded.to_type, to_id=excluded.to_id, relation_type=excluded.relation_type, tradition_scope=excluded.tradition_scope, confidence=excluded.confidence, source_refs_json=excluded.source_refs_json, status='active';`);
...
function buildSql(bundle) {
  ...
  return buildSqlForBundle({
    ...
    contentRelations: bundle.objects?.[`${slug}ContentRelations`] ?? [],
    objects: bundle.objects?.[`${slug}MythicObjects`] ?? [],
